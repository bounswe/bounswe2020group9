...
urlpatterns = [
    ...
    # POST(private), DELETE(private) # TODO add/remove product to/from list
    path('cart/', CartAPIView().as_view(), name="user-cart"), #GET(private) # TODO return cart of user, note it is private
    path('<int:id>/alert_list/', AlertListAPIView().as_view(), name="user-list-alerted_list"), #GET(private) # TODO return alerted list of user, note it is private
